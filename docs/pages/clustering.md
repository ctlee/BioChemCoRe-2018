...
`cluster.log` is not the only file that is generated as output. There is a pdb file that contains the pdb files of all the cluster centroids. Also make sure to check out the several .xmg files generated as output files. You can open them and see the plots by typing:
`xmgrace FILENAME.xmg`
